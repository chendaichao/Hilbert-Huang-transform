...
from scipy.signal import hilbert, argrelmin, argrelmax
...
def find_IMF(x, tolerance = 0.0002, num_extrema = 3):
    '''
        ...
        x : array_like, 1D
            Signal data. must be real.
        tolerance : real, optional. ( Default: 0.0002 )
            The threshold used in the stopping criteria for the sifting process.
            When the mean squared relative difference between two consecutive 
    ...
    while (True):
        maxima = argrelmax(x, mode="warp")[0]
        minima = argrelmin(x, mode="warp")[0]
        if (len(maxima) <= num_extrema) or (len(minima) <= num_extrema):
            return None
        
        t = list(range(N))
        x_maxima, x_minima = x[maxima], x[minima]
        tck_up, tck_down = interpolate.splrep(minima, x_minima, k=3), interpolate.splrep(maxima, x_maxima, k=3)
        ...
        h = x - mean
        if ( ((mean / (x + 1e-6))**2).mean() < tolerance ):
            break
        x = h 
        
    return h

def emd(x, eps = 0.01, max_num_imf = 20, tolerance = 0.0002, num_extrema = 3, ret_residual = False):
    '''
...
def plot_IMFs(x, imfs, delta_t = None, fs = None, index = None, time_range = None, save_fig = None):
    '''
    ...
    ax.plot(t, x[L, R] if time_range else x)
    ...
    for i_ in range(len(index)):
        i = index[i_]
        imf = imfs[i]
        scale = np.abs(imf[L, R] if time_range else imf).max()
        ax = plt.subplot(num_imfs+2, 1, i_+2)
        ax.plot(t, imf[L, R] if time_range else imf)
...
def hilbert_huang(x, delta_t = None, fs = None, eps = 0.01, 
                  max_num_imf = 20, tolerance = 0.0002, num_extrema = 3, freq_unit = 'Hz'):
    '''
        ...
        (imfs, imfs_env, imfs_freq) : list of numpy.ndarray
            ( N = len(x), nIMF = #IMFs ) 
            imfs : 2-D numpy.ndarray of shape (nIMF+1, N)
                IMFs and the residual obtained from `emd`.
            imfs_env : 2-D numpy.ndarray of shape (nIMF, N-1)
                The envelope functions of all IMFs.
            imfs_freq : 2-D numpy.ndarray of shape (nIMF, N-1)
                The instantaneous frequency functions of all IMFs.
    '''
    assert (delta_t is not None) or (fs is not None), "One of `fs` and `delta_t` must be specified."
    if (delta_t is None):
        delta_t = 1. / fs
    imfs = np.stack(emd(x, eps, max_num_imf, tolerance, num_extrema))
    imfs_env, imfs_freq = get_envelope_frequency(imfs[:-1], delta_t, freq_unit = freq_unit)
...
def hilbert_spectrum(imfs_env, imfs_freq, delta_t = None, fs = None, freq_unit = 'Hz', freq_lim = None, 
    ...
    if (freq_lim is None):
        ...
        while (low + freq_res < high):
            mid = (low + high)/2
            if (( (imfs_freq<=mid) * imfs_env**2).sum() <= 0.999 * totalE):
                low = mid
            else:
                high = mid
        freq_lim = high
    ...
    spectrum = torch.sparse.DoubleTensor(N, M+1)
    imfs_freq, imfs_env = torch.from_numpy(imfs_freq), torch.from_numpy(imfs_env).double()
    ...
    for i in range(num_imf):
        ...
        spectrum.add_(torch.sparse.DoubleTensor(idx, value, torch.Size([N, M+1])))
    
    spectrum = spectrum.coalesce()
    
    if (display):
        
        time_axis = np.arange(L, R) * delta_t if time_range else np.arange(N) * delta_t
        freq_axis = np.arange(M+1) * freq_res
        
        spectrum_, time_axis_, freq_axis_ = _shrink(spectrum, time_axis, freq_axis)
        spectrum_ = spectrum_.to_dense().numpy()
        eps = spectrum_.max() * (1e-5)
        coutour = plt.pcolormesh(time_axis_, freq_axis_, 10*np.log10(spectrum_.T + eps), cmap = plt.cm.YlGnBu_r)
        ...
        marginal = torch.sparse.sum(spectrum, dim=0).to_dense().view(-1)
        #plt.plot(freq_axis, 10*np.log10(marginal + 1e-10))
        plt.plot(freq_axis, marginal)
